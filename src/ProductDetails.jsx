...
function ProductDetails() {
  ...
  const handleAddToCart = () => {
    ...
    // گرفتن رنگ انتخاب‌شده
    const selectedColor = document.querySelector('input[name="color"]:checked')?.id || "";

    // بررسی وجود محصول با همان id و رنگ
    const existingItem = cartItems.find(
      (item) => item.id === product.id && item.color === selectedColor
    );

    if (existingItem) {
      // اگر وجود داشت تعداد زیاد میشه
      existingItem.quantity += quantity;
    } else {
      // اگر نبود اضافه میشه
      cartItems.push({ ...product, quantity, color: selectedColor });
    }
...
console.log("Product state:", product);
  ...
  <div className="row">
    {/* ستون aside سمت چپ برای فیلتر */}
    <aside className="col-md-3" style={{ borderLeft: "1px solid #ddd" }}>
      <DropdownFilter />
    </aside>

    {/* بخش محصول: عکس و جزئیات */}
      ...
      <div className="row">
        ...
        <div className="col-md-6">
          ...
          <div className="d-grid gap-2">
            <button className="btn btn-custom" type="button" onClick={handleAddToCart}>
              Add to Cart
            </button>
